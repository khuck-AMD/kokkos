...
list(APPEND SUPPORTED_AMD_GPUS GCNSPIRV)
list(APPEND SUPPORTED_AMD_ARCHS AMD_GCNSPIRV)
list(APPEND CORRESPONDING_AMD_FLAGS amdgcnspirv)
...
function(CHECK_AMDGPU_ARCH ARCH FLAG)
  if(KOKKOS_ARCH_${ARCH})
    if(NOT KOKKOS_ENABLE_HIP AND NOT KOKKOS_ENABLE_OPENACC AND NOT KOKKOS_ENABLE_SYCL)
      ...
      list(APPEND AMDGPU_ARCH_ALREADY_SPECIFIED ${ARCH})
      set(AMDGPU_ARCH_ALREADY_SPECIFIED ${AMDGPU_ARCH_ALREADY_SPECIFIED} PARENT_SCOPE)
      list(FIND KOKKOS_AMDGPU_ARCH_FLAGS ${FLAG} AMDGPU_FLAG_INDEX)
      if(AMDGPU_FLAG_INDEX EQUAL -1)
        list(APPEND KOKKOS_AMDGPU_ARCH_FLAGS ${FLAG})
        set(KOKKOS_AMDGPU_ARCH_FLAGS ${KOKKOS_AMDGPU_ARCH_FLAGS} PARENT_SCOPE)
        list(JOIN KOKKOS_AMDGPU_ARCH_FLAGS "," KOKKOS_AMDGPU_ARCH_FLAG)
        set(KOKKOS_AMDGPU_ARCH_FLAG ${KOKKOS_AMDGPU_ARCH_FLAG} PARENT_SCOPE)
      endif()
      if(KOKKOS_ENABLE_HIP)
        set(KOKKOS_HIP_ARCHITECTURES ${KOKKOS_AMDGPU_ARCH_FLAGS} PARENT_SCOPE)
      endif()
...
#Regardless of version, make sure we define the general architecture name
set(KOKKOS_ARCH_AMD_GPU_LIST)
foreach(ARCH IN LISTS SUPPORTED_AMD_ARCHS)
  if(KOKKOS_ARCH_${ARCH})
    list(FIND SUPPORTED_AMD_ARCHS ${ARCH} LIST_INDEX)
    list(GET CORRESPONDING_AMD_FLAGS ${LIST_INDEX} FLAG)
    list(APPEND KOKKOS_ARCH_AMD_GPU_LIST "${FLAG}")
...
endforeach()
if(KOKKOS_ARCH_AMD_GPU_LIST)
  list(REMOVE_DUPLICATES KOKKOS_ARCH_AMD_GPU_LIST)
  list(JOIN KOKKOS_ARCH_AMD_GPU_LIST "," KOKKOS_ARCH_AMD_GPU)
endif()
...
if(Kokkos_ENABLE_HIP)
  foreach(arch IN LISTS GPU_TARGETS)
    list(FIND KOKKOS_HIP_ARCHITECTURES ${arch} KOKKOS_GPU_TARGET_MATCH_INDEX)
    if(KOKKOS_GPU_TARGET_MATCH_INDEX EQUAL -1)
      ...
      message(
        ${MESSAGE_TYPE}
        "AMD GPU architectures given via GPU_TARGETS=\"${GPU_TARGETS}\" are not compatible with the architectures enabled in Kokkos which are ${KOKKOS_HIP_ARCHITECTURES}. Please configure with GPU_TARGETS as a subset of ${KOKKOS_HIP_ARCHITECTURES}."
      )
      break()
    endif()
  endforeach()
